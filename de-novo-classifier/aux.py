...
def readVcfToDF(fname, sample_list=None, chunk_size=None):
...
def df2sklearn(mydf, col_to_keep):
    """This assumes that mydf has a lable column named status
    and positive/negative examples are marked with Y/N.
    Returns numeric df with specified features, and 0/1 labels"""
...
def runInShell(cmd, return_output=False):
    """Run a shell command, and optionally
    return output"""
...
def runBamReadcounts(vcffile, bamfile, output_dir,
    ...
    tmp_bed = tempfile.mktemp(dir=tmp_dir,
                              suffix='.bed',
                              prefix=bam_name+'_')
...
def parseBamReadcountIndel(row):
    ...
    res = pandas.Series(ref_split + [num_allels, '_'.join(alts),
                                     alt_read_count] +
                        cum_array.tolist(),
